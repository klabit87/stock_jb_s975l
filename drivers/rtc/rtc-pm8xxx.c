...
pm8xxx_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	if ( sapa_saved_time.enabled ) {
		...
		if ( secs_pwron < secs ) {
			pr_info("replaced to pwr on alarm\n");
			secs = secs_pwron; 
			override = 1;
		}
	}
...
#ifdef CONFIG_RTC_AUTO_PWRON
	if ( override == 1 ) {
		dev_info(dev, "Alarm Set for h:r:s=%d:%d:%d, d/m/y=%d/%d/%d (override)\n",
			sapa_saved_time.time.tm_hour, sapa_saved_time.time.tm_min,
			sapa_saved_time.time.tm_sec, sapa_saved_time.time.tm_mday,
			sapa_saved_time.time.tm_mon, sapa_saved_time.time.tm_year);
	}
	else 
#endif
	{
